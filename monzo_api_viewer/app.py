...
from monzo_api_viewer.helpers.currencies import currencies
from markupsafe import escape
from monzo_api_viewer.helpers.monzo_api_storage import FileSystem
...
def pots_for_account():
    """Handle displaying pots for an account."""
    if 'account' in request.form and escape(request.form['account']):
        account = escape(request.form['account'])
        if account == 'Please Select':
            context = {
    ...
    try:
        ...
        return render_template('pots.html', **context)
    except MonzoPermissionsError:
        error = (
                'The API returned a permissions issue. Either the token has expired or the account cannot return '
                + 'transactions.'
